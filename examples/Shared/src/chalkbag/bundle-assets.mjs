...
copy(path.join(FIX, 'wall/wall.glb'), 'cb_wall.glb')
...
const routeEntries = picked.map((r) =>
  `  { id: ${JSON.stringify(r.id)}, name: ${JSON.stringify(r.name)}, circuit: ${JSON.stringify(r.circuit)}, holdIds: ${JSON.stringify(r.holdIds)} },`
).join('\n')
